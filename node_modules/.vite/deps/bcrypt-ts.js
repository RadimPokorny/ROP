...
var f = (n) => Array(n).fill(-1);
var E = [...f(46), 0, 1, ...I.slice(54, 64), ...f(7), ...I.slice(2, 28), ...f(6), ...I.slice(28, 54), ...f(5)];
var L = [608135816, 2242054355, 320440878, 57701188, 2752067618, 698298832, 137296536, 3964562569, 1160258022, 953160567, 3193202383, 887688300, 3232508343, 3380367581, 1065670069, 3041331479, 2450970073, 2306472731];
...
var N = [1332899944, 1700884034, 1701343084, 1684370003, 1668446532, 1869963892];
var S = (n, r) => {
  if (r <= 0 || r > n.length)
...
var O = (n, r) => {
  if (r <= 0)
    throw Error(`Illegal len: ${r}`);
  const o = n.length;
  let t = 0, e = 0, s, l, h, u, i, a;
  const g = [];
  for (; t < o - 1 && e < r && (a = n.charCodeAt(t++), s = a < E.length ? E[a] : -1, a = n.charCodeAt(t++), l = a < E.length ? E[a] : -1, !(s == -1 || l == -1 || (i = s << 2 >>> 0, i |= (l & 48) >> 4, g.push(String.fromCharCode(i)), ++e >= r || t >= o) || (a = n.charCodeAt(t++), h = a < E.length ? E[a] : -1, h == -1) || (i = (l & 15) << 4 >>> 0, i |= (h & 60) >> 2, g.push(String.fromCharCode(i)), ++e >= r || t >= o))); )
    a = n.charCodeAt(t++), u = a < E.length ? E[a] : -1, i = (h & 3) << 6 >>> 0, i |= u, g.push(String.fromCharCode(i)), ++e;
...
var w = typeof process == "object" && process.env.NEXT_RUNTIME === "edge" ? setTimeout : typeof setImmediate == "function" ? setImmediate : typeof process == "object" && typeof process.nextTick == "function" ? process.nextTick : setTimeout;
var k = (n) => {
  const r = [];
  let o = 0;
...
var b = (n, r, o) => {
  ...
  let s = 0, l = [0, 0], h;
  for (let u = 0; u < t; u++)
    h = _(n, s), s = h.offp, r[u] = r[u] ^ h.key;
  for (let u = 0; u < t; u += 2)
    l = T(l, 0, r, o), r[u] = l[0], r[u + 1] = l[1];
  for (let u = 0; u < e; u += 2)
    l = T(l, 0, r, o), o[u] = l[0], o[u + 1] = l[1];
};
var F = (n, r, o, t) => {
  const e = o.length, s = t.length;
  let l = 0, h = [0, 0], u;
  for (let i = 0; i < e; i++)
    u = _(r, l), l = u.offp, o[i] = o[i] ^ u.key;
  l = 0;
  for (let i = 0; i < e; i += 2)
    u = _(n, l), l = u.offp, h[0] ^= u.key, u = _(n, l), l = u.offp, h[1] ^= u.key, h = T(h, 0, o, t), o[i] = h[0], o[i + 1] = h[1];
  for (let i = 0; i < s; i += 2)
    u = _(n, l), l = u.offp, h[0] ^= u.key, u = _(n, l), l = u.offp, h[1] ^= u.key, h = T(h, 0, o, t), t[i] = h[0], t[i + 1] = h[1];
};
var R = (n, r, o, t, e) => {
  const s = N.slice(), l = s.length;
  if (o < 4 || o > 31) {
  ...
  o = 1 << o >>> 0;
  let h, u, i = 0, a;
  Int32Array ? (h = new Int32Array(L), u = new Int32Array(C)) : (h = L.slice(), u = C.slice()), F(r, n, h, u);
  const g = () => {
    if (e && e(i / o), i < o) {
      const c = Date.now();
      for (; i < o && (i = i + 1, b(n, h, u), b(r, h, u), !(Date.now() - c > 100)); )
        ;
    } else {
      for (i = 0; i < 64; i++)
        for (a = 0; a < l >> 1; a++)
      ...
      const c = [];
      for (i = 0; i < l; i++)
        c.push((s[i] >> 24 & 255) >>> 0), c.push((s[i] >> 16 & 255) >>> 0), c.push((s[i] >> 8 & 255) >>> 0), c.push((s[i] & 255) >>> 0);
      return t === false ? Promise.resolve(c) : c;
    }
    if (t === false)
...
var G = (n) => {
  var _a;
  try {
    const { crypto: r, msCrypto: o } = window, t = new Uint32Array(n);
    return (_a = r || o) == null ? void 0 : _a.getRandomValues(t), Array.from(t);
  } catch {
...
function d(n, r, o, t) {
  ...
  const l = parseInt(r.substring(s, s + 1), 10) * 10, h = parseInt(r.substring(s + 1, s + 2), 10), u = l + h, i = r.substring(s + 3, s + 25);
  n += e >= "a" ? "\0" : "";
  const a = k(n), g = O(i, 16), c = (p) => {
    const y = [];
    return y.push("$2"), e >= "a" && y.push(e), y.push("$"), u < 10 && y.push("0"), y.push(u.toString()), y.push("$"), y.push(S(g, g.length)), y.push(S(p, N.length * 4 - 1)), y.join("");
  };
  return o === false ? R(a, g, u, false, t).then((p) => c(p)) : c(R(a, g, u, true, t));
}
var U = (n, r = 10) => {
  if (typeof r == "number" && (r = m(r)), typeof n != "string" || typeof r != "string")
    throw Error("Illegal arguments: " + typeof n + ", " + typeof r);
  return d(n, r, true);
};
var P = function(n, r, o) {
  return typeof n == "string" && typeof r == "number" ? $(r).then((t) => d(n, t, false, o)) : typeof n == "string" && typeof r == "string" ? d(n, r, false, o) : Promise.reject(new Error(`Illegal arguments: ${typeof n}, ${typeof r}`));
};
var x = (n, r) => {
  if (typeof n != "string" || typeof r != "string")
    throw Error("Illegal arguments: " + typeof n + ", " + typeof r);
  return r.length !== 60 ? false : U(n, r.substring(0, r.length - 31)) === r;
};
...
var M = (n) => {
  if (typeof n != "string")
    throw new Error(`Illegal arguments: ${typeof n}`);
...
var X = (n) => {
  if (typeof n != "string")
    throw new Error(`Illegal arguments: ${typeof n}`);
  if (n.length !== 60)
    throw new Error(`Illegal hash length: ${n.length} != 60`);
